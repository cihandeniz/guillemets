...
internal class PropertyResolver(VariableStore _variables, Glossary _glossary)
{
    public IEnumerable<IDataSource> Resolve(Scope scope, PropertyChainNode properties) =>
        new PropertyChainResolution(scope, properties, _variables, _glossary).Resolve();
    ...
    {
        var resolution = new PropertyChainResolution(scope, properties, _variables, _glossary);
