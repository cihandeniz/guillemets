...
left*, not a history of what's done. For *how* it's built, see
`ARCHITECTURE.md`; for *how* to work (TDD discipline, code style), see
`CLAUDE.md`.
...
name only). `parameters` comes before `variable-definitions`:
...
to land first regardless of fixture-number order on disk (`/specs` is
already renumbered to match: `05-parameters` … `08-inline-lists`).

1. `parameters` — `format`/`currency`/`length`. First milestone that
   needs typed (`DateTime`/`decimal`) access rather than just display
   strings/booleans — this is where the deferred `IDataSource`
   typed-access question (see `ARCHITECTURE.md`) gets decided,
   test-first.
2. `variable-definitions` — `definition-boolean` and `definition-object`
   done; `definition-list-separator` needs `(separator = , )` parameter
   parsing from the milestone above, so do it last.
3. `tables` — should mostly fall out of the above once blocks exist;
   confirm rather than build new.
4. `inline-lists` — field-selection projection and custom `(separator)`
   already work for `variables/nested-property-chained-list`-style
   cases; confirm the remaining fixtures, particularly the
